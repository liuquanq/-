...
/* 删除111回合后的平局判定，删除以禁解禁，删除死四判定 */

int Judge::Rule_order(int color, int row, char col)	//行棋顺序判定
{
	int order_judge = 0;
	//判定是否为棋局第一子
	//第一回合
...
int Judge::Rule_disallowed_moves(Arraybeta a, int row, int col)//禁手裁决
{
	//int count = 1;
	//int h, s, z, f = 1;
	//横竖正斜反斜四个方向,以当前子为基点，已有计数1
	...
	//不可返回多个值
	//return long_result,double_3_result,double_4_result;
	//无触犯禁手,0
	return 0;
}

int Judge::Alive_2(Arraybeta a, int row, int col, char flag, int color)
{
	//返回0是非二,返回1是活二，2是死二
	int result = 0;
	int i = 1;
...
int Judge::Alive_3(Arraybeta a, int row, int col, char flag, int color)
{
	//返回0是非三，1是活三，2为死三
	int result = 0;
	int count = 1;//连子长度
...
int Judge::Alive_4(Arraybeta a, int row, int col, char flag, int color)
{
	//返回0是非四,返回1是活四，2是死四
	int result = 0;
	int i = 1;
...
int Judge::Double_2_judge(Arraybeta a, int row, int col, int color)
{
	int result = 0;//1为双活二
	int count = 0;
...
int Judge::Double_3_judge(Arraybeta a, int row, int col, int color)
{
	//双活三,不考虑以禁解禁
	int result = 0;//-1为触犯禁手
	int count = 0;//活三计数
...
int Judge::Double_4_judge(Arraybeta a, int row, int col, int color)
{
	//双四
...
int Judge::LongConnect_judge(Arraybeta a, int row, int col, int count, char flag)
{
	int i = 1;
	int result = 0;
...
int Judge::Judgement(Arraybeta a, int row, int col, int color)
{
	//判断胜负
	int result = 0;//0为未分胜负,1为当前落子获胜,2为平局
	int j = 0;//循环四个方向
